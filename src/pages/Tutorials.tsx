  ...
  return (
    <div className="min-h-screen pt-24 pb-12 px-4 md:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading && [1,2,3].map(i => (
              <div key={i} className="bg-[#1a1a1a] rounded-2xl h-64 animate-pulse border border-white/5" />
          ))}
          {!loading && tutorials.map((tut) => (
              <div 
                  key={tut.id} 
              ...
              {!loading && tutorials.length === 0 && <div className="col-span-full text-center py-20 text-gray-500">No tutorials found.</div>}
      </div>
    </div>
  );
}
